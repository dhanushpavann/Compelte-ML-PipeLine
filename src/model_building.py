...
def load_data(file_path : str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.debug("Loaded loaded from %s with shape %s",file_path,df.shape)
        return df
...
def train_model(X_train : np.ndarray, y_train : np.ndarray, params: dict) -> RandomForestClassifier:
    ...
    try:
        ...
        logger.debug("Value Error during model training: %s",e)
        raise
    except Exception as e:
        logger.debug("Error occured during model training: %s",e)
        raise
...
def save_model(model,file_path : str) -> None:
    ...
    try:
        ...
        logger.debug("Error while saving model: %s",e)
        raise
